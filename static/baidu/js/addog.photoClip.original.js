...
(function (root, factory) {
    ...
    function photoClip(container, option) {
        var size = option.size,
            ...
            beforeLoadStart = option.beforeLoadStart,
        ...
        $file.attr("accept", "image/jpg, image/jpeg, image/png, image/svg, image/bmp");
        $file.off("change").on("change", function () {
            if (!this.files.length) return;
            var files = this.files[0];
            console.log(files.type);
            if (!/image\/\w+/.test(files.type)) {
                //if (files.type.indexOf('.gif') > -1) {
                //alert("图片格式不正确，请选择正确格式的图片文件！");
                $('.tipsPopup').css('display', 'block');
                $('#tipsPopupOne').html('图片格式不正确');
                $('#tipsPopupTwo').html('请选择正确格式的图片文件');
                setTimeout(function () {
                ...
                if (files.type == 'image/gif') {
                    ...
                    let isValid = beforeLoadStart.call(files);
                    /*let isOss = get_signature();*/
                    if (isValid) {
        ...
        var $ok = $(ok);
        if ($ok.length) {
            $ok.off('click').on('click', function () { //先解绑,再绑定事件
                clipImg();
            });
        }
        ...
        function clipImg() {
            // imgLoad = imgLoaded;
            if (!imgLoaded) { //图片是否已经加载完成了  没有加载完成 说明 没有 选择过 图片,不裁剪和上传oss
                if (newItem) {
                    if ($('#addWebUrl').val() == '') {
                        $('.tipsSpanOne').css('display', 'block');
                    } else {
                        $('.tipsSpanOne').css('display', 'none');
                        console.log('没有选择图片时，添加新网址校验网址是否合法');

                        //网址的合法性校验
                        var isUrl = IsWebURL($('#addWebUrl').val());
                        // console.log(isUrl);
                        if(isUrl){
                            // console.log('输入网址合法');
                            $('.tipsSpanIsUrl').css('display', 'none');
                            if ($('#addWebName').val() == '') {
                                $('.tipsSpanTwo').css('display', 'block');
                            } else {
                                $('.tipsSpanTwo').css('display', 'none');
                                if (app.imgArrTotal <= 99) {
                                    //console.log(g_object_name);
                                    $.ajax({  //添加收藏案例
                                        type: 'POST',
                                        url: '/sv1/guider/add',
                                        data: {
                                            icon: g_object_name,  // logo 可不传
                                            iconUrl: '',
                                            auGuiderDesc: $('#addWebDetail').val(), //描述  可不传
                                            auGuiderName: $('#addWebName').val(), //名称  必传
                                            auGuiderUrl: checkUrl($('#addWebUrl').val()),  //链接  必传
                                            // auGuiderUrl: $('#addWebUrl').val(),  //链接  必传
                                        },
                                        dataType: 'JSON',
                                        success: function (data) {
                                            if (data.status === 0) {
                                                clipAreaT = null;
                                                fileImg = null;
                                                $('#addWebview').css('background-image', 'none');
                                                $('#addWebclipArea').css('background-image', 'none');
                                                $('#addWebfile').val('');
                                                app.imgArrTotal = app.imgArrTotal + 1;

                                                $('#addWebsite').css('display', 'none');
                                                $('#shade').css('display', 'none');

                                                app.imgArr.splice(app.imgArr.length - 1, 0, data.data);
                                                app.arrPositionInit();
                                                localStorageArr = JSON.stringify(app.imgArr);
                                                window.localStorage.collList = localStorageArr;
                                                initializationData();
                                            }
                                        }
                                    });
                                    ...
                                    $('.uplimit').css('display', 'block');
                                }
                            }
                        }else{
                            $('.tipsSpanIsUrl').css('display', 'block');
                            // console.log('输入的网址不合法');
                        }
                    }
                    ...
                    if ($('#modifyUrl').val() == '') {
                        ...
                        console.log('没有选择图片时，修改网址校验网址是否合法');
                        var isModifyUrl = IsWebURL($('#modifyUrl').val());
                        if(isModifyUrl){
                            // alert('yes');
                            $('.tipsSpanModifyUrl').css('display', 'none');
                            if ($('#modifyName').val() == '') {
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        /*icon:g_object_name,  // logo 可不传
                                        iconUrl:iconUrl,*/
                                        auGuiderIndex: deleObj.auGuiderIndex,
                                        auUuid: deleObj.auUuid,
                                        auGuiderDesc: $('#modifyDetail').val(), //描述  可不传
                                        auGuiderName: $('#modifyName').val(), //名称  必传
                                        auGuiderUrl: checkUrl($('#modifyUrl').val()),  //链接  必传
                                        // auGuiderUrl: $('#modifyUrl').val(),  //链接  必传
                                    },
                                    dataType: 'JSON',
                                    success: function (data) {
                                        if (data.status === 0) {
                                            ...
                                            app.imgArr.splice(modifyIndex, 1, data.data);
                                            app.arrPositionInit();
                            ...
                            $('.tipsSpanModifyUrl').css('display', 'block');
                        }
                    }
                }
