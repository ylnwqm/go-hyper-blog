(function($, window, document) {
    ...
    $.extend({
        ...
        showmsg: function(msg) {
            ...
            w_m.appendTo('body').css({
                left: Math.floor(($(window).width() - $('.w_p').innerWidth()) / 2) + 'px',
                top: $(window).scrollTop() + Math.floor(($(window).height() - $('.w_p').innerHeight()) / 2) + 'px'
            }).animate({
                opacity: 1
            }, 1000);
                ...
                var uploader = new qq.FileUploader({
                    ...
                    onProgress: function(id, filename, loaded, total) {
                        if (typeof ops.pro === 'function') {
                            ops.pro(id, filename, loaded, total);
                            return;
                        }
            ...
            try {
                var uploader = new qq.FileUploader({
                    ...
                    onProgress: function(id, filename, loaded, total) {
                        ...
                        t.find('.bar').css('width', (loaded / total) * 100 + '%').find('.del').bind('click', function() {
                            t.remove()
                        });
                        return;
                    },
    ...
    $.fn.getValid = function(ops) {
        var s = $.extend({}, {
            url: '/HomeAsync/SendMobile',
            resend_url: '/HomeAsync/Resend',
            type: 1
        }, ops);
    ...
    $.fn.cm = function() {
        return this.each(function(i, v) {
            $(v).css({
                position: 'absolute',
                zIndex: 999,
                top: $(window).scrollTop() + Math.round(($(window).height() - $(v).outerHeight()) / 2),
                left: Math.round(($(window).width() - $(v).outerWidth()) / 2)
            });
        });
    }
    $.fn.loading = function() {
    ...
    $.fn.city = function() {
        return this.each(function(i, v) {
            ...
            var f_p = $($.lightbox({
                title: '更改所在地',
                content: '<div class="mb20"><label class="form_m">现在所在地</label></div><div class="city"><select id="province" name="province"></select><label>省</label><select id="city" name="city"></select><label>市</label></div><span class="Validform_checktip"></span></div>',
                btn: '更改'
            })).appendTo('body');
            f_p.find('.f_p').cm();
            f_p.find('.sub').bind('click', function() {
                $.post('/HomeAsync/SetAddr', {
                    province: f_p.find('select:eq(0)').val(),
                    city: f_p.find('select:eq(1)').val()
                }, function(data) {
                    if ($.parseJSON(data).Status == 0) {
                        f_p.remove();
                        $.showmsg($.parseJSON(data).Msg);
    ...
    $.fn.printCity = function() {
        return this.each(function() {
            ...
            $(this).find('#province').bind('change', function() {
                s();
            });
            var s1 = $('select')[0];
            var s2 = $('select')[1];
    ...
    $.fn.unfollow = function() {
        return this.each(function(i, v) {
            ...
            var f_p = $($.lightbox({
                title: '取消关注',
                content: '<span>你确定取消对<b style="color:green">' + s.nickName + '</b>的关注么？</span>',
                btn: '确定'
            }));
    ...
    $.fn.unfans = function() {
        return this.each(function() {
            ...
            var f_p = $($.lightbox({
                title: '移除粉丝',
                content: '<span>你确定要移除<b style="color:green">' + s.nickName + '</b>？</span>',
                btn: '确定'
            }));
    ...
    $.fn.addBlack = function() {
        return this.each(function(i, v) {
            ...
            var f_p = $($.lightbox({
                title: '加入黑名单',
                content: '<span>确认将<b style="color:green">' + s.nickName + '</b>加入到黑名单中么？</span><p>你和他将自动解除关注关系, 将不能继续关注！</p>',
                btn: '加入黑名单'
            }));
            $(v).find('a[action-type=add_black]').bind('click', function() {
                f_p.appendTo('body').find('.lightBox').light().next('.f_p').cm();
            });
            f_p.find('.c').bind('click', function() {
                f_p.detach();
            });
            f_p.find('.sub').click(function() {
                f_p.detach();
                $.post('/HomeAsync/ToBlack', {
                    ID: s.userID
                }, function(data, textStatus, xhr) {
                    var d = $.parseJSON(data);
                    if (d.Status === 0) {
    ...
    $.fn.removeBlack = function() {
        return this.each(function(i, v) {
            ...
            $(v).find('.s_btn').bind('click', function() {
                var f_p = $($.lightbox({
                    title: '移除黑名单',
                    content: '<span>确认将<b style="color:green">' + s.nickName + '</b>移除黑名单？</span>',
                    btn: '移除黑名单'
                }));
                f_p.appendTo('body').find('.lightBox').light().next('.f_p').cm();
                f_p.find('.c').bind('click', function() {
    ...
    $.fn.random = function() {
        return this.each(function(i, v) {
            ...
            var temp = '<li class="p bt clear" action-data="userID=$UserID$&nickName=$NickName$">' +
                ...
                '</div></div>' +
                '</li>'
            $.get('/HomeAsync/GetInterestingUser', {
                type: at,
                page: $(v).data('page')
            }, function(data) {
                var d = $.parseJSON(data);
    ...
    $.fn.moveUp = function() {
        return this.each(function(i, v) {
            ...
            var _o = {
                top: _n.attr('data-top') + 'px',
                down: _n.attr('data-down') + 'px'
            };
            _t.hover(function() {
                _n.stop().animate({
                    top: _o.top
...
$(document).ready(function() {
    ...
    $('.fn_nav_child').hover(function() {
        $(this).find('.f_icon').stop().animate({
            'backgroundPositionY': '-61px',
            'backgroundColor': '#333024'
        }, 300);
    }, function() {
        $(this).find('.f_icon').stop().animate({
    ...
    $('#sites').children().each(function(i, v) {
        $(v).hover(function() {
            $('#sites').next().children().eq(i).animate({
                top: 60,
                opacity: 1
            }).show();
        }, function() {
